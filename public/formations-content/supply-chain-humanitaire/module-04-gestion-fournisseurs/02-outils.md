...
**Fiche de qualification & évaluation fournisseur (`fiche-qualification-evaluation-fournisseur.xlsx`)** — le formulaire de validation fournisseur de ce module. Il regroupe l'identification du fournisseur (contact, visite des locaux, dates de validation, contrat, référent technique) et une évaluation structurée en 29 critères répartis sur 5 catégories — financier, légal, produits, service, prix — chacun noté Oui/Non via une liste déroulante. Un panneau de synthèse en haut de la feuille utilise une formule COUNTIF pour afficher en temps réel le nombre de critères validés sur 29 et le taux de réponses complètes, avant la décision finale de validation et la signature conjointe du membre supply et du référent technique.
...
Ouvrez le formulaire `fiche-qualification-evaluation-fournisseur.xlsx` et complétez le bloc **Identification du fournisseur**. Réunissez, au minimum : preuve d'existence légale (certificat d'enregistrement / immatriculation), certificat fiscal à jour, coordonnées bancaires (RIB), au moins deux à trois références clients vérifiables, et une preuve de capacité opérationnelle réelle (visite de site si le volume d'achat le justifie). Un dossier incomplet n'est pas qualifié "sous réserve" — il n'est pas qualifié, point final, tant que les pièces manquantes ne sont pas obtenues.

**Étape 2 — Effectuer le screening d'inéligibilité avant toute signature**

Avec le nom exact du fournisseur ET le nom de ses dirigeants/propriétaires identifiés au dossier, effectuez la recherche dans cet ordre : d'abord un balayage large sur **OpenSanctions.org** pour repérer rapidement tout signal, puis, en cas de résultat — même approximatif — confirmation systématique sur la liste primaire concernée (OFAC, UE ou ONU selon l'origine du signal). Ne jamais fonder une décision de blocage ou de validation uniquement sur l'agrégateur : c'est la liste officielle qui fait foi. Consignez la date de la recherche, les listes consultées et le résultat directement dans le formulaire (champ Date de validation du fournisseur et zone Commentaires additionnels) — un screening non daté n'a aucune valeur probante en cas d'audit.
...
Une fois le fournisseur actif, reprenez le formulaire et parcourez les 29 critères répartis en 5 catégories (financier, légal, produits, service, prix), en répondant Oui/Non à chacun via la liste déroulante. Le panneau **Synthèse** en haut de la feuille calcule automatiquement, par formule COUNTIF, le nombre de critères validés sur 29 et le taux de réponses complètes — un coup d'œil suffit pour repérer un dossier fournisseur qui se dégrade. Cette évaluation n'est pas un exercice ponctuel : elle doit être répétée à fréquence régulière (tous les 6 à 12 mois selon le volume d'achat), pas seulement lors d'un incident.

**Étape 5 — Suivre les échéances de réévaluation dans le temps**

Chaque formulaire porte son propre champ **Date de la prochaine validation** : reportez-la dans votre suivi de portefeuille fournisseurs (tableur ou outil de suivi dédié) pour visualiser d'un coup d'œil qui arrive à échéance. Un fournisseur dont la date de prochaine validation est dépassée doit être réévalué avant toute nouvelle commande significative — pas après.
