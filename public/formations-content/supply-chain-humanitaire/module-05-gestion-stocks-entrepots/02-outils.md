# Module 05 — Gestion des Stocks & Entrepôts
...
**Étape 9 — Documenter toute sortie de stock, y compris le carburant, avec la même rigueur que la réception**

Tout ce qui entre dans l'entrepôt via le bon de réception et contrôle qualité doit ressortir tracé de la même façon. Pour toute sortie de marchandise — livraison à un partenaire, transfert vers un autre site, remise à une équipe terrain — complétez le template `bons-de-sortie-entrepot-carburant.xlsx` fourni avec ce module, onglet « Bon de sortie entrepôt » : il reprend la structure d'un Delivery Note (DN), avec la référence de la demande interne, le point de stockage d'origine, la destination, et la signature des quatre parties concernées (approbation, responsable stock, transporteur, receveur) avant que le véhicule ne quitte l'entrepôt. Pour toute distribution de carburant à un véhicule, utilisez l'onglet « Bon de carburant » du même classeur : un bon par plein, avec kilométrage au compteur et quantité délivrée, à classer avec le carnet de bord du véhicule concerné. Comme pour la réception, aucune sortie ne doit quitter l'entrepôt sans ce document complété et signé — c'est cette discipline symétrique, à l'entrée comme à la sortie, qui rend la réconciliation d'inventaire de l'Étape 7 réellement fiable.
